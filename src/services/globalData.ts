...
import type {
  LifeOSGlobalComparisonResult, LifeOSGlobalComparisonStatus, LifeOSGlobalDataResult, LifeOSGlobalMetric,
  LifeOSGlobalMetricDefinition, LifeOSGlobalMetricQuery, LifeOSGlobalMetricType, LifeOSGlobalPeriod,
  LifeOSGlobalRankingBand, LifeOSGlobalRankingBandResult, LifeOSGlobalRankingDistribution,
  LifeOSGlobalRankingDistributionBand, LifeOSGlobalChallenge, LifeOSGlobalChallengeStatus,
  LifeOSGlobalCommunityAggregate, LifeOSGlobalParticipationStatus, LifeOSGlobalTrend,
} from '@/types/global';
...
const PERIODS: LifeOSGlobalPeriod[] = ['day', 'week', 'month', 'quarter', 'year'];
const TYPES: LifeOSGlobalMetricType[] = ['count', 'sum', 'average', 'rate', 'ratio'];
...
export const LIFEOS_GLOBAL_METRIC_DEFINITIONS: readonly LifeOSGlobalMetricDefinition[] = [];

const unavailable = (error: string): LifeOSGlobalDataResult => ({ status: 'unavailable', metrics: [], error });
const invalid = (error: string): LifeOSGlobalDataResult => ({ status: 'invalid', metrics: [], error });

function validDate(value: string): boolean {
  return /^\d{4}-\d{2}-\d{2}$/.test(value) && !Number.isNaN(Date.parse(`${value}T00:00:00Z`));
}

function validQuery(query: LifeOSGlobalMetricQuery): boolean {
  if (query.metricName !== undefined && !LIFEOS_GLOBAL_METRIC_DEFINITIONS.some((d) => d.metricName === query.metricName)) return false;
...
function isMetricShapeValid(metric: LifeOSGlobalMetric): boolean {
...
function mapMetric(row: unknown): LifeOSGlobalMetric | null {
  if (!row || typeof row !== 'object' || Array.isArray(row)) return null;
  const r = row as Record<string, unknown>;
  if (typeof r.metric_name !== 'string' || !TYPES.includes(r.metric_type as LifeOSGlobalMetricType) ||
      !PERIODS.includes(r.period as LifeOSGlobalPeriod) || typeof r.period_start !== 'string' || !validDate(r.period_start) ||
  ...
  const definition = LIFEOS_GLOBAL_METRIC_DEFINITIONS.find((d) => d.metricName === r.metric_name && d.metricType === r.metric_type);
  if (!definition || !definition.supportedPeriods.includes(r.period as LifeOSGlobalPeriod)) return null;
  return {
    metricName: r.metric_name, metricType: r.metric_type as LifeOSGlobalMetricType,
    period: r.period as LifeOSGlobalPeriod, periodStart: r.period_start, periodEnd: r.period_end,
    ...(r.geographic_level ? { geographicLevel: r.geographic_level as LifeOSGlobalMetric['geographicLevel'] } : {}),
...
export async function readGlobalMetrics(query: LifeOSGlobalMetricQuery = {}): Promise<LifeOSGlobalDataResult> {
  ...
  try {
    let request = supabase.from('lifeos_global_metrics').select(GLOBAL_METRIC_COLUMNS);
    ...
    return metrics.length ? { status: 'available', metrics: metrics as LifeOSGlobalMetric[], error: null } : { status: 'empty', metrics: [], error: null };
  } catch { return unavailable('Global data is currently unavailable.'); }
}

function previousEquivalentPeriod(metric: LifeOSGlobalMetric): { start: string; end: string } | null {
...
const comparisonUnavailable = (status: LifeOSGlobalComparisonStatus, error: string | null = null): LifeOSGlobalComparisonResult => ({ status, comparison: null, trend: 'insufficient_data', error });
function sameMetricIdentity(left: LifeOSGlobalMetric, right: LifeOSGlobalMetric): boolean { return left.metricName === right.metricName && left.metricType === right.metricType && left.period === right.period; }
function sameGeography(left: LifeOSGlobalMetric, right: LifeOSGlobalMetric): boolean { return left.geographicLevel === right.geographicLevel && left.geographicCode === right.geographicCode; }
function compatibleGeographies(left: LifeOSGlobalMetric, right: LifeOSGlobalMetric): boolean { return sameGeography(left, right) || left.geographicLevel === 'global' || right.geographicLevel === 'global'; }
function buildComparison(current: LifeOSGlobalMetric, previous: LifeOSGlobalMetric, kind: 'period' | 'geographic'): LifeOSGlobalComparisonResult {
  ...
  const trend: LifeOSGlobalTrend = absoluteChange > 0 ? 'up' : absoluteChange < 0 ? 'down' : 'unchanged';
  return { status: 'available', comparison: { current, previous, absoluteChange, percentageChange, kind }, trend, error: null };
}

export async function compareGlobalMetric(metric: LifeOSGlobalMetric, previous: LifeOSGlobalMetric | null): Promise<LifeOSGlobalComparisonResult> {
...
export function compareGlobalPeriods(current: LifeOSGlobalMetric, previous: LifeOSGlobalMetric | null): LifeOSGlobalComparisonResult {
...
export function compareGlobalAggregates(current: LifeOSGlobalMetric, reference: LifeOSGlobalMetric | null): LifeOSGlobalComparisonResult {
  if (!isMetricShapeValid(current)) return comparisonUnavailable('invalid', 'Invalid current global metric.');
...
function validRankingBand(band: unknown): band is LifeOSGlobalRankingBand { return band === 'bottom' || band === 'lower' || band === 'middle' || band === 'upper' || band === 'top'; }
function validateRankingDistribution(distribution: LifeOSGlobalRankingDistribution): LifeOSGlobalRankingDistributionBand[] | null {
  ...
  const expected: LifeOSGlobalRankingBand[] = ['bottom', 'lower', 'middle', 'upper', 'top'];
...
export function calculateGlobalRankingBand(value: LifeOSGlobalMetric, distribution: LifeOSGlobalRankingDistribution | null): LifeOSGlobalRankingBandResult {
...
function validChallengeStatus(status: unknown): status is LifeOSGlobalChallengeStatus { return status === 'upcoming' || status === 'active' || status === 'completed' || status === 'unavailable' || status === 'invalid'; }
function validGlobalChallengeShape(challenge: LifeOSGlobalChallenge): boolean {
...
export function getGlobalChallengeLifecycle(challenge: LifeOSGlobalChallenge, today: string): LifeOSGlobalChallengeStatus {
...
export function validateGlobalChallenge(challenge: LifeOSGlobalChallenge): boolean { return validGlobalChallengeShape(challenge); }
export function isGlobalChallengeMetricCompatible(challenge: LifeOSGlobalChallenge, metric: LifeOSGlobalMetric): boolean {
...
export function validateGlobalCommunityAggregate(aggregate: LifeOSGlobalCommunityAggregate): boolean {
...
export async function getGlobalParticipationStatus(challenge: LifeOSGlobalChallenge): Promise<LifeOSGlobalParticipationStatus> {
