...
import type { BackupValidationResult, LifeOSBackup } from '@/types/backup';
...
export function useBackup() {
  ...
  const exportUserBackup = useCallback(async (): Promise<LifeOSBackup | null> => {
    setBusy(true); setMessage(null); setError(null);
    try {
      const backup = await exportBackup();
      const filename = `lifeos-backup-${backup.createdAt.slice(0, 10)}.json`;
      const uri = `${FileSystem.cacheDirectory ?? FileSystem.documentDirectory ?? ''}${filename}`;
      await FileSystem.writeAsStringAsync(uri, serializeBackup(backup));
      if (await Sharing.isAvailableAsync()) {
        await Sharing.shareAsync(uri, { mimeType: 'application/json', dialogTitle: 'Export LifeOS Backup' });
  ...
  const importUserBackup = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      setMessage('Restore completed. LifeOS will reload from the restored source data.');
      return true;
    } catch {
