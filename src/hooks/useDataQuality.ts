...
import type { DataQualityModel } from '@/types/dataQuality';
import { todayCivilDate } from '@/lib/date';

export function useDataQuality(initialDate: string = todayCivilDate()) {
  const [date, setDate] = useState(initialDate);
  const [data, setData] = useState<DataQualityModel | null>(null);
  ...
  const load = useCallback(async (nextDate: string = date) => {
    ...
    setError(null);
    try {
      const result = await getDataQuality(nextDate);
      ...
      const message = cause instanceof Error ? cause.message : 'Failed to load data quality diagnostics';
      setError(message);
      throw cause;
    } finally {
