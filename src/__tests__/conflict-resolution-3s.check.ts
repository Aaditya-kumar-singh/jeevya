// JEEVYA 3S: deterministic conflict detection and resolution coverage.
// @ts-nocheck
...
(async () => {
  ...
  const remoteOf = (payload, deleted = false) => ({ id: `remote-${payload?.id ?? base.id}`, user_id: 'user-3s', domain: 'tasks', record_id: payload?.id ?? base.id, storage_key: 'jeevya:tasks', payload: deleted ? null : payload, updated_at: payload?.updatedAt ?? '2026-09-14T08:00:00.000Z', deleted, sync_version: 1, device_updated_at: payload?.updatedAt ?? '2026-09-14T08:00:00.000Z' });
  const resetToBaseline = async (baseline = base) => { await AsyncStorage.clear(); await saveData('jeevya:tasks', [baseline]); remoteRows = [remoteOf(baseline)]; upserts = []; await sync.synchronizeJeevya(); upserts = []; };

  await check('baseline unchanged is a no-op', async () => {
    await saveData('jeevya:tasks', [base]); upserts = []; remoteRows = []; await sync.synchronizeJeevya(); remoteRows = upserts.map((row) => ({ ...row, id: 'remote-baseline' })); upserts = [];
    const result = await sync.synchronizeJeevya(); assert(result.state === 'synced' && result.conflicts.length === 0 && upserts.length === 0, 'baseline no-op failed');
  });

  await check('local-only change uploads', async () => {
    const local = { ...base, title: 'Local only', updatedAt: '2026-09-14T09:00:00.000Z' }; await saveData('jeevya:tasks', [local]); upserts = [];
    const result = await sync.synchronizeJeevya(); assert(result.state === 'synced' && upserts.some((row) => row.payload.title === 'Local only'), 'local-only upload failed');
  });

  await check('remote-only change applies locally', async () => {
    await resetToBaseline();
    const remote = { ...base, title: 'Remote only', updatedAt: '2026-09-14T10:00:00.000Z' }; await saveData('jeevya:tasks', [base]); remoteRows = [remoteOf(base)]; upserts = []; await sync.synchronizeJeevya(); remoteRows = [remoteOf(remote)]; upserts = [];
    const result = await sync.synchronizeJeevya(); assert(result.state === 'synced' && (await loadData('jeevya:tasks', []))[0].title === 'Remote only', 'remote-only apply failed');
  });
  ...
  await check('both changed creates a persistent conflict from baseline', async () => {
    ...
    await saveData('jeevya:tasks', [{ ...baseline, title: 'Local conflict', updatedAt: '2026-09-14T12:00:00.000Z' }]); remoteRows = [remoteOf({ ...baseline, title: 'Remote conflict', updatedAt: '2026-09-14T13:00:00.000Z' })];
    const result = await sync.synchronizeJeevya(); const pending = await sync.getPendingConflicts(); const conflict = pending[0];
  ...
  await check('keep-local resolves and synchronizes exactly once', async () => {
    ...
    assert(result.success && result.state === 'synced', `keep-local failed: ${result.message}`); assert((await sync.getPendingConflicts()).length === 0, 'conflict not cleared'); assert((await loadData('jeevya:tasks', []))[0].title === 'Local conflict', 'local version lost');
  ...
  await check('local delete plus remote update becomes deletion conflict', async () => {
    ...
    await saveData('jeevya:tasks', []); remoteRows = [remoteOf({ ...baseline, title: 'Remote update', updatedAt: '2026-09-14T15:00:00.000Z' })];
    const result = await sync.synchronizeJeevya(); const conflict = (await sync.getPendingConflicts()).find((x) => x.recordId === base.id); assert(result.state === 'conflict_pending' && conflict?.reason === 'deletion_conflict' && conflict.localDeleted, 'delete/update conflict missing');
  });
  ...
  await check('keep-remote safely restores the remote update', async () => {
    ...
    assert(result.success && (await loadData('jeevya:tasks', []))[0].title === 'Remote update', 'keep-remote failed'); assert((await sync.getPendingConflicts()).length === 0, 'remote conflict not cleared');
  });
  ...
  await check('local update plus remote delete becomes deletion conflict', async () => {
    ...
    const local = { ...baseline, title: 'Keep me', updatedAt: '2026-09-14T17:00:00.000Z' }; await saveData('jeevya:tasks', [local]); remoteRows = [remoteOf(baseline, true)];
    const result = await sync.synchronizeJeevya(); const conflict = (await sync.getPendingConflicts()).find((x) => x.recordId === base.id); assert(result.state === 'conflict_pending' && conflict?.reason === 'deletion_conflict' && conflict.remoteDeleted, 'update/delete conflict missing');
  });
  ...
  await check('keep-local on delete conflict preserves data and uploads update', async () => {
    ...
    assert(result.success && (await loadData('jeevya:tasks', []))[0].title === 'Keep me' && upserts.some((row) => !row.deleted && row.payload.title === 'Keep me'), 'keep-local delete conflict failed');
  });
  ...
  await check('both deleted is not a conflict', async () => {
    ...
    await saveData('jeevya:tasks', []); remoteRows = [remoteOf(baseline, true)]; const result = await sync.synchronizeJeevya(); assert(result.state === 'synced' && (await sync.getPendingConflicts()).length === 0, 'both delete became conflict');
  });

  await check('invalid remote creates safe pending diagnostic and does not overwrite local', async () => {
    await resetToBaseline();
    await saveData('jeevya:tasks', [base]); remoteRows = [{ id: 'bad', user_id: 'user-3s', domain: 'tasks', record_id: base.id, storage_key: 'jeevya:tasks', payload: 'bad', updated_at: '2026-09-14T19:00:00.000Z', deleted: false }];
    const result = await sync.synchronizeJeevya(); const conflict = (await sync.getPendingConflicts()).find((x) => x.reason === 'invalid_remote'); const current = await loadData('jeevya:tasks', []); assert(result.state === 'error' && conflict && current[0]?.title === 'Baseline', `invalid remote was not safely isolated: ${JSON.stringify(result)} conflict=${JSON.stringify(conflict)} current=${JSON.stringify(current)}`);
  ...
  await check('malformed conflict metadata is diagnosed without making empty state unhealthy', async () => {
    await AsyncStorage.setItem('jeevya:sync:conflicts', JSON.stringify([{ conflictId: 'bad', domain: 'tasks', status: 'wrong' }]));
    ...
    await AsyncStorage.removeItem('jeevya:sync:conflicts'); assert((await sync.getSyncConflictDiagnostics()).malformed === 0, 'empty conflict state unhealthy');
  });

  await check('restore preserves local data and leaves conflict detectable until explicit resolution', async () => {
    await resetToBaseline();
    const backup = await exportBackup(); await saveData('jeevya:tasks', [{ ...base, title: 'Restored', updatedAt: '2026-09-14T20:00:00.000Z' }]); await restoreBackup(backup); assert((await loadData('jeevya:tasks', []))[0].title === 'Baseline', 'restore changed authoritative local state unexpectedly');
  });
  ...
  await check('concurrent sync and resolution do not corrupt the local record', async () => {
    ...
    await saveData('jeevya:tasks', [{ ...baseline, title: 'Concurrent local', updatedAt: '2026-09-14T22:00:00.000Z' }]); remoteRows = [remoteOf({ ...baseline, title: 'Concurrent remote', updatedAt: '2026-09-14T23:00:00.000Z' })]; await sync.synchronizeJeevya();
    ...
    assert(ra.success && rb.success && (await loadData('jeevya:tasks', []))[0].title === 'Concurrent local', 'concurrent resolution corrupted local state');
  });
  ...
  await check('failed resolution remains retryable and preserves local data', async () => {
    ...
    await saveData('jeevya:tasks', [{ ...baseline, title: 'Failure local', updatedAt: '2026-09-15T00:00:00.000Z' }]); remoteRows = [remoteOf({ ...baseline, title: 'Failure remote', updatedAt: '2026-09-15T01:00:00.000Z' })]; await sync.synchronizeJeevya();
    ...
    assert(!result.success && (await loadData('jeevya:tasks', []))[0].title === 'Failure local' && (await sync.getPendingConflicts()).some((x) => x.conflictId === conflict.conflictId), 'failed resolution was lost');
  ...
  await check('multiple independent conflicts remain isolated', async () => {
    await resetToBaseline();
    const a = { ...base, id: 'task-a', title: 'A', updatedAt: '2026-09-15T02:00:00.000Z' }; const b = { ...base, id: 'task-b', title: 'B', updatedAt: '2026-09-15T02:00:00.000Z' }; await saveData('jeevya:tasks', [a, b]); remoteRows = [remoteOf(a), remoteOf(b)]; await sync.synchronizeJeevya();
    await saveData('jeevya:tasks', [{ ...a, title: 'A local', updatedAt: '2026-09-15T03:00:00.000Z' }, { ...b, title: 'B local', updatedAt: '2026-09-15T03:00:00.000Z' }]); remoteRows = [remoteOf({ ...a, title: 'A remote', updatedAt: '2026-09-15T04:00:00.000Z' }), remoteOf({ ...b, title: 'B remote', updatedAt: '2026-09-15T04:00:00.000Z' })]; const result = await sync.synchronizeJeevya(); assert(result.conflicts.length >= 2, 'independent conflicts were not isolated');
  });

  await check('no unauthenticated cloud mutation remains', async () => {
    Object.defineProperty(supabase.auth, 'getSession', { configurable: true, writable: true, value: async () => ({ data: { session: null }, error: null }) }); upserts = []; const before = JSON.stringify(await loadData('jeevya:tasks', [])); const result = await sync.synchronizeJeevya(); assert(result.state === 'auth_required' && upserts.length === 0 && JSON.stringify(await loadData('jeevya:tasks', [])) === before, 'unauthenticated mutation occurred');
  });

  console.log(`JEEVYA 3S CONFLICT RESOLUTION: ${passed} passed, 0 failed`);
