// LIFEOS 3N: cross-module backup, validation, replace restore, and rollback coverage.
// @ts-nocheck
...
(async () => {
  ...
  await check('empty-state backup', async () => {
    ...
    assert(backup.format === 'lifeos-backup' && backup.version === 1, 'metadata');
  ...
  await check('populated multi-domain backup and domain inclusion', async () => {
    await saveData('lifeos:tasks', [task]); await saveData('lifeos:labels', [label]); await saveData('lifeos:habits', [habit]);
    await saveData('lifeos:books', [book]); await saveData('lifeos:finance:accounts', [account]); await saveData('lifeos:finance:categories', [category]);
    await saveData('lifeos:finance:transactions', [transaction]); await saveData('lifeos:health:sleep', [sleep]);
    const backup = await exportBackup();
  ...
  await check('deterministic output structure and preserved records', async () => {
    ...
    assert(a.domains.tasks['lifeos:tasks'][0].id === task.id, 'ID not preserved');
    assert(a.domains.tasks['lifeos:tasks'][0].createdAt === task.createdAt, 'timestamp not preserved');
  });
  ...
  await check('unsupported future version rejection', async () => assert(!validateBackup({ format: 'lifeos-backup', version: 99, createdAt: new Date().toISOString(), appVersion: '1', domains: {} }).valid, 'future version accepted'));
  await check('malformed domain rejection', async () => assert(!validateBackup({ format: 'lifeos-backup', version: 1, createdAt: new Date().toISOString(), appVersion: '1', domains: { tasks: [] } }).valid, 'malformed domain accepted'));
  await check('invalid IDs and dates rejection', async () => {
    const bad = { format: 'lifeos-backup', version: 1, createdAt: new Date().toISOString(), appVersion: '1', domains: { tasks: { 'lifeos:tasks': [{ id: '', dueDate: 'not-a-date' }] } } };
  ...
  await check('invalid relationships rejection', async () => {
    const bad = { format: 'lifeos-backup', version: 1, createdAt: new Date().toISOString(), appVersion: '1', domains: { tasks: { 'lifeos:tasks': [{ id: 't', labelIds: ['missing'] }] } } };
  ...
  await check('preflight validation does not mutate data', async () => {
    const before = await loadData('lifeos:tasks', []); const result = await preflightBackup(JSON.stringify(await exportBackup())); const after = await loadData('lifeos:tasks', []);
  ...
  await check('successful restore with replace semantics', async () => {
    ...
    await saveData('lifeos:tasks', [{ ...task, id: 'old-task', title: 'Old' }]);
    await saveData('lifeos:journal', [{ id: 'stale-journal', date: '2026-09-14', title: 'stale' }]);
    const result = await restoreBackup(source);
    assert(result.success, result.message);
    const restored = await loadData('lifeos:tasks', []); const stale = await loadData('lifeos:journal', []);
  ...
  await check('restored cross-domain relationships and regenerated projections', async () => {
    const restored = await loadData('lifeos:tasks', []); const labels = await loadData('lifeos:labels', []); const tx = await loadData('lifeos:finance:transactions', []);
    assert(restored[0].labelIds[0] === labels[0].id && tx[0].accountId === account.id, 'relationships not restored');
    const state = await getLifeOSDailyState('2026-09-14'); const plan = buildDailyPlan(state);
  ...
  await check('missing optional domains remain valid', async () => {
    const minimal = { format: 'lifeos-backup', version: 1, createdAt: new Date().toISOString(), appVersion: '1', domains: { tasks: { 'lifeos:tasks': [task], 'lifeos:labels': [label] } } };
    assert(validateBackup(minimal).valid, 'optional domains incorrectly required');
  });

  await check('duplicate prevention', async () => {
    const duplicate = { format: 'lifeos-backup', version: 1, createdAt: new Date().toISOString(), appVersion: '1', domains: { tasks: { 'lifeos:tasks': [task, task] } } };
  ...
  await check('failed restore preserves existing data through rollback', async () => {
    const original = await loadData('lifeos:tasks', []);
    const source = await exportBackup();
    ...
    const after = await loadData('lifeos:tasks', []);
  ...
  await check('no destructive mutation behavior outside explicit replace restore', async () => {
    const before = await loadData('lifeos:books', []);
    await preflightBackup(await exportBackup());
    const after = await loadData('lifeos:books', []);
    assert(JSON.stringify(before) === JSON.stringify(after), 'validation changed books');
  });

  await AsyncStorage.clear();
  console.log(`LIFEOS 3N BACKUP: ${passed} passed, 0 failed`);
})();
