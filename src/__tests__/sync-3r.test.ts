// LIFEOS 3R: Supabase persistence/sync foundation coverage.
// @ts-nocheck
...
(async () => {
  ...
  const { synchronizeLifeOS, getSyncStatus } = await import('@/services/sync');
  ...
  await check('no authenticated session is explicit and non-mutating', async () => {
    ...
    const before = JSON.stringify(await loadData('lifeos:tasks', []));
    const result = await synchronizeLifeOS();
    const after = JSON.stringify(await loadData('lifeos:tasks', []));
  ...
  await check('authenticated session uploads authoritative local records', async () => {
    ...
    remoteRows = []; upserts = [];
    const localBeforeSync = await loadData('lifeos:tasks', []);
    const result = await synchronizeLifeOS();
  ...
  await check('repeated sync is idempotent', async () => {
    ...
    const first = await synchronizeLifeOS();
  ...
  await check('remote-only record downloads without changing native shape', async () => {
    ...
    remoteRows = [{ id: 'remote-row-2', user_id: 'user-3r', domain: 'tasks', record_id: remoteTask.id, storage_key: 'lifeos:tasks', payload: remoteTask, updated_at: remoteTask.updatedAt, deleted: false, sync_version: 2, device_updated_at: remoteTask.updatedAt }];
    upserts = [];
    const result = await synchronizeLifeOS();
    const tasks = await loadData('lifeos:tasks', []);
  ...
  await check('local newer is uploaded rather than overwritten', async () => {
    ...
    await saveData('lifeos:tasks', [localNewer]);
    remoteRows = [{ id: 'remote-row-2', user_id: 'user-3r', domain: 'tasks', record_id: localNewer.id, storage_key: 'lifeos:tasks', payload: remoteTask, updated_at: remoteTask.updatedAt, deleted: false, sync_version: 2, device_updated_at: remoteTask.updatedAt }];
    upserts = [];
    const metadataBefore = await loadData('lifeos:sync:metadata', null);
    const localBefore = await loadData('lifeos:tasks', []);
    ...
    assert((await loadData('lifeos:tasks', []))[0].title === 'Local newer', 'local data overwritten');
  });
  ...
  await check('possible conflict is detected without applying remote blindly', async () => {
    ...
    await saveData('lifeos:tasks', [baseline]);
    remoteRows = [{ id: 'remote-row-3', user_id: 'user-3r', domain: 'tasks', record_id: baseline.id, storage_key: 'lifeos:tasks', payload: { ...baseline, title: 'Remote changed', updatedAt: '2026-09-14T12:00:00.000Z' }, updated_at: '2026-09-14T12:00:00.000Z', deleted: false, sync_version: 3, device_updated_at: '2026-09-14T12:00:00.000Z' }];
    upserts = [];
    const result = await synchronizeLifeOS();
    assert(result.state === 'conflict_pending' && result.conflicts.length >= 1, 'conflict not detected');
    assert((await loadData('lifeos:tasks', []))[0].title === 'Baseline', 'conflicting remote applied');
  });
  ...
  await check('network failure remains offline and preserves local data', async () => {
    ...
    assert((await loadData('lifeos:tasks', []))[0].id === task.id, 'offline data changed');
  });

  await check('malformed remote payload is rejected safely', async () => {
    remoteRows = [{ id: 'bad', user_id: 'user-3r', domain: 'tasks', record_id: 'bad', storage_key: 'lifeos:tasks', payload: 'not-an-object', updated_at: '2026-09-14T12:00:00.000Z', deleted: false, sync_version: 1 }];
    const result = await synchronizeLifeOS();
    assert(result.state === 'error' && (await loadData('lifeos:tasks', []))[0].id === task.id, 'malformed remote mutated local data');
  });

  await check('unsupported remote domain is rejected', async () => {
    remoteRows = [{ id: 'bad', user_id: 'user-3r', domain: 'analytics', record_id: 'x', storage_key: 'lifeos:analytics', payload: {}, updated_at: '2026-09-14T12:00:00.000Z', deleted: false, sync_version: 1 }];
    const result = await synchronizeLifeOS();
  ...
  await check('deletion tombstone is preserved and applied only when safe', async () => {
    remoteRows = [{ id: 'remote-row-3', user_id: 'user-3r', domain: 'tasks', record_id: task.id, storage_key: 'lifeos:tasks', payload: task, updated_at: task.updatedAt, deleted: false, sync_version: 3, device_updated_at: task.updatedAt }];
    await saveData('lifeos:tasks', [task]);
    await synchronizeLifeOS();
    remoteRows = [{ id: 'remote-row-4', user_id: 'user-3r', domain: 'tasks', record_id: task.id, storage_key: 'lifeos:tasks', payload: null, updated_at: '2026-09-14T13:00:00.000Z', deleted: true, sync_version: 4, device_updated_at: '2026-09-14T13:00:00.000Z' }];
    const result = await synchronizeLifeOS();
    const current = await loadData('lifeos:tasks', []); assert(result.state === 'synced' && current.length === 0, `safe tombstone was not applied: ${JSON.stringify(result)} current=${JSON.stringify(current)}`);
  });
  ...
  await check('concurrent sync calls are serialized', async () => {
    await AsyncStorage.clear(); await saveData('lifeos:tasks', [task]);
    remoteRows = []; upserts = []; selectDelay = 15;
    const [a, b] = await Promise.all([synchronizeLifeOS(), synchronizeLifeOS()]);
    selectDelay = 0;
  ...
  await check('sync during local write cannot overwrite the newer local state', async () => {
    ...
    const syncing = synchronizeLifeOS();
    await new Promise((resolve) => setTimeout(resolve, 5));
    const newer = { ...task, title: 'Written during sync', updatedAt: '2026-09-14T14:00:00.000Z' };
    await saveData('lifeos:tasks', [newer]);
    await syncing; selectDelay = 0;
    assert((await loadData('lifeos:tasks', []))[0].title === 'Written during sync', 'sync overwrote concurrent local write');
  });

  await check('restore followed by explicit sync keeps restored local data safe', async () => {
    await AsyncStorage.clear(); await saveData('lifeos:tasks', [{ ...task, title: 'Restored local' }]);
    const backup = await exportBackup();
    await saveData('lifeos:tasks', [{ ...task, title: 'Other local' }]);
    const restored = await restoreBackup(backup);
    assert(restored.success && (await loadData('lifeos:tasks', []))[0].title === 'Restored local', 'restore failed');
    remoteRows = []; upserts = [];
    const result = await synchronizeLifeOS();
  ...
  console.log(`LIFEOS 3R SYNC FOUNDATION: ${passed} passed, 0 failed`);
